...
from src.attributes.stochastic_models import GeometricBrownianMotion, OrnsteinUhlenbeck

# Test configuration constants
N_POINTS = 100_000  # Large dataset for reliable parameter estimation
DT = 1 / (24 * 60 * 60)  # 1 second time step
...
def assert_within_tolerance(fitted_value: float, true_value: float, tolerance: float = TOLERANCE):
    """
...
def test_gbm_parameter_recovery_standard():
    ...
    simulated_prices = gbm.simulate(
        ...
        X_0=INITIAL_PRICE,
        dt=DT
    )[0]  # Take first (and only) path
    
    # Fit the simulated data
    gbm_fit = GeometricBrownianMotion()
    mu_fitted, sigma_fitted, log_likelihood = gbm_fit.fit(simulated_prices, DT)
...
def test_gbm_simulated_mean_properties():
    ...
    std_error = sigma * np.sqrt(DT) / np.sqrt(len(log_returns))
...
def test_gbm_positive_prices():
    ...
    assert np.all(np.isfinite(simulated_prices)), (
        "GBM produced NaN or Inf values"
    )
...
def test_ou_parameter_recovery_standard():
    ...
    mu_true = 1.0  # mean reversion speed
    ...
    mu_fitted, theta_fitted, sigma_fitted, log_likelihood = ou_fit.fit(simulated_values, DT)
    
    # Assert parameters are recovered within tolerance
    assert_within_tolerance(mu_fitted, mu_true)
    assert_within_tolerance(theta_fitted, theta_true, tolerance=0.20)  # Mean harder to estimate
    assert_within_tolerance(sigma_fitted, sigma_true)
    assert np.isfinite(log_likelihood)
