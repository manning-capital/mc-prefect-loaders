...
def compare_dataframes(
    ...
    if not records_in_1_not_2.empty:
        ...
        records_in_1_not_2 = records_in_1_not_2.drop(columns=[
            col for col in records_in_1_not_2.columns 
            if col.endswith("_2") or col == "_merge"
        ])
        records_in_1_not_2.columns = [col.replace("_1", "") for col in records_in_1_not_2.columns]
    ...
    if not records_in_2_not_1.empty:
        ...
        records_in_2_not_1 = records_in_2_not_1.drop(columns=[
            col for col in records_in_2_not_1.columns 
            if col.endswith("_1") or col == "_merge"
        ])
        records_in_2_not_1.columns = [col.replace("_2", "") for col in records_in_2_not_1.columns]
    ...
    if not common_records.empty:
        ...
        if not exact_matches.empty:  # type: ignore
            ...
            exact_matches = exact_matches.drop(columns=[  # type: ignore
                col for col in exact_matches.columns  # type: ignore
        ...
        if not different_records.empty:  # type: ignore
            ...
            different_records = different_records.drop(columns=[  # type: ignore
                col for col in different_records.columns  # type: ignore
                if col.endswith("_1") or col.endswith("_2") or col == "_merge"
            ])
    else:
        # No common records
...
@task()
async def set_data(table_name: str, data: pd.DataFrame, operation_type: Literal["insert", "append", "upsert"] = "upsert"):
